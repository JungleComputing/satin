...
public class KMeans extends ibis.satin.SatinObject implements KMeansInterface {
    ...
    @Override
    public KMeansResult kMeanCluster(double[][] centers, Points points, int lo, int high) {
    ...
    public static void main(String[] args) throws Exception {
        int numTasks = 1;
        double centers[][] = readCenters(args[0]);
        ...
        do {
            ...
            for (int i = 0; i < centers.length; i++) {
                if (results[0].counts[i] > 0) {
                    total += eucledianDistance(centers[i], results[0].centers[i]);
                    centers[i] = results[0].centers[i];
                }
            }
            System.out.println("Iteration " + iteration + ", total diff = " + total);
