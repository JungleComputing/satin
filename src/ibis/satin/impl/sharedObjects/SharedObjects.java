...
public final class SharedObjects implements Config {
    ...
    private Vector<SOInvocationRecord> soInvocationList = new Vector<>();

    private Satin s;

    /**
     * A hash containing all shared objects: (String objectID, SharedObject object)
     */
    private HashMap<String, SharedObjectInfo> sharedObjects = new HashMap<>();
     ...
     */
    public void setSOReference(String objectId, IbisIdentifier source) throws SOReferenceSourceCrashedException {
    ...
    /**
     * Add a shared object invocation record to the so invocation record list; the
     * invocation will be executed later
     */
    ...
    /**
     * Execute the guard of the invocation record r, wait for updates, if necessary,
     * ship objects if necessary
     */
    private void doExecuteGuard(InvocationRecord r) throws SOReferenceSourceCrashedException {
        ...
        soLogger.info("SATIN '" + s.ident + "': " + "guard not satisfied, getting updates..");

        // try to ship the object(s) from the owner of the job
        Vector<String> objRefs = r.getSOReferences();
        if (objRefs == null || objRefs.isEmpty()) {
            soLogger.error("SATIN '" + s.ident + "': " + "a guard is not satisfied, but the spawn does not " + "have shared objects.\n"
    ...
    public void broadcastSOInvocation(SOInvocationRecord r) {
        ...
        if (so != null && so.isUnshared()) {
            ...
            soLogger.debug("No broadcast from writeMethod: object " + r.getObjectId() + " is not shared yet");
            return;
        }
    ...
    boolean waitForObject(String objectId, IbisIdentifier source, InvocationRecord r, long timeout) {
        ...
        while (true) {
            ...
            if (r == null) {
                if (s.so.getSOInfo(objectId) != null) {
                    soLogger.debug("SATIN '" + s.ident + "': received new object from a bcast");
                    return true; // got it!
                }
            } else if (r.guard()) {
                soLogger.debug("SATIN '" + s.ident + "': received object, guard satisfied");
                return true;
            }
        }
    }
