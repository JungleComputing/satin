...
final class SOCommunication implements Config, Protocol {
    ...
    private ArrayList<IbisIdentifier> toConnect = new ArrayList<>();

    private HashMap<IbisIdentifier, ReceivePortIdentifier> ports = new HashMap<>();
    ...
    protected void init() {
        ...
        if (LABEL_ROUTING_MCAST) {
            try {
                ...
                SOInvocationHandler soInvocationHandler = new SOInvocationHandler(s);

                // Create a multicast port to bcast shared object invocations.
                // Connections are established later.
                soSendPort = s.comm.ibis.createSendPort(soPortType, "lrmc port");
                soReceivePort = s.comm.ibis.createReceivePort(soPortType, "lrmc port", soInvocationHandler);
                ...
                if (SO_MAX_INVOCATION_DELAY > 0) {
                    ...
                    soInvocationHandler.setMessageSplitter(new MessageSplitter(props, soReceivePort));
                }
                soReceivePort.enableConnections();
                soReceivePort.enableMessageUpcalls();
            } catch (Exception e) {
                commLogger.error("SATIN '" + s.ident + "': Could not start ibis: " + e, e);
            ...
            try {
                ...
                // Create a multicast port to bcast shared object invocations.
                // Connections are established later.
                soSendPort = s.comm.ibis.createSendPort(soPortType, "satin so port on " + s.ident);
    ...
    public static PortType getSOPortType() throws IOException {
        if (LABEL_ROUTING_MCAST) {
            return new PortType(PortType.CONNECTION_MANY_TO_MANY, PortType.RECEIVE_EXPLICIT, PortType.RECEIVE_AUTO_UPCALLS,
                    PortType.SERIALIZATION_OBJECT);
        }
        return new PortType(PortType.CONNECTION_ONE_TO_MANY, PortType.CONNECTION_UPCALLS, PortType.CONNECTION_DOWNCALLS, PortType.RECEIVE_EXPLICIT,
                PortType.RECEIVE_AUTO_UPCALLS, PortType.SERIALIZATION_OBJECT);
    }

    /**
     * Creates SO receive ports for new Satin instances. Do this first, to make them
    ...
    protected void handleJoins(IbisIdentifier[] joiners) {
        ...
        if (LABEL_ROUTING_MCAST) {
            ...
            synchronized (s) {
                for (IbisIdentifier joiner : joiners) {
                    toConnect.add(joiner);
                }
            }
            return;
        }
        ...
        for (IbisIdentifier joiner : joiners) {
            ...
            try {
                s.comm.ibis.createReceivePort(soPortType, "satin so receive port for " + joiner, new SOInvocationHandler(s),
    ...
    protected void sendAccumulatedSOInvocations() {
        ...
        if (soInvocationsDelayTimer > 0 && (elapsed > SO_MAX_INVOCATION_DELAY || soCurrTotalMessageSize > SO_MAX_MESSAGE_SIZE)) {
            try {
                s.stats.broadcastSOInvocationsTimer.start();

                soMessageCombiner.sendAccumulatedMessages();
            } catch (IOException e) {
                System.err.println("SATIN '" + s.ident + "': unable to broadcast shared object invocations " + e);
            } finally {
    ...
    protected void broadcastSOInvocation(SOInvocationRecord r) {
        ...
        } else if (ASYNC_SO_BCAST) {
            // We have to make a copy of the object first, the caller might
            // modify it.
            SOInvocationRecord copy = (SOInvocationRecord) Satin.deepCopy(r);
    ...
    protected void doBroadcastSOInvocationLRMC(SOInvocationRecord r) {
        ...
        soBcastLogger.debug("SATIN '" + s.ident + "': broadcasting so invocation for: " + r.getObjectId());
        ...
        try {
            ...
            if (soSendPort != null) {
                try {
                    ...
                    System.err.println("SATIN '" + s.ident + "': unable to broadcast a shared object invocation: " + e);
                }
                if (SO_MAX_INVOCATION_DELAY > 0) {
    ...
    /**
     * This basicaly is optional, if nodes don't have the object, they will retrieve
     * it. However, one broadcast is more efficient (serialization is done only
     * once). We MUST use message combining here, we use the same receiveport as the
     * SO invocation messages. This is only called by exportObject.
     */
    ...
    protected void doBroadcastSharedObject(SharedObject object) {
        ...
        try {
            ...
            try {
                ...
                System.err.println("SATIN '" + s.ident + "': unable to broadcast a shared object: " + e);
            }
    ...
    protected void doBroadcastSharedObjectLRMC(SharedObject object) {
        ...
        soBcastLogger.info("SATIN '" + s.ident + "': broadcasting object: " + object.getObjectId());
        WriteMessage w = null;
        long size = 0;
    ...
    /**
     * Fetch a shared object from another node. If the Invocation record is null,
     * any version is OK, we just test that we have a version of the object. If it
     * is not null, we try to satisfy the guard of the invocation record. It might
     * not be satisfied when this method returns, the guard might depend on more
     * than one shared object.
     */
    protected void fetchObject(String objectId, IbisIdentifier source, InvocationRecord r) throws SOReferenceSourceCrashedException {

        if (s.so.waitForObject(objectId, source, r, SO_WAIT_FOR_UPDATES_TIME)) {
            return;
        }

        soLogger.debug("SATIN '" + s.ident + "': did not receive object in time, demanding it now");
        ...
        soLogger.error("SATIN '" + s.ident + "': internal error: did not receive shared object after I demanded it. ");
    }

    private void sendSORequest(String objectId, IbisIdentifier source, boolean demand) throws SOReferenceSourceCrashedException {
        ...
        try {
            ...
            if (v == null) {
                ...
                soLogger.error("SATIN '" + s.ident + "': could not " + "write shared-object request");
            ...
            // hm we've got a problem here
            // push the job somewhere else?
            soLogger.error("SATIN '" + s.ident + "': could not " + "write shared-object request", e);
            throw new SOReferenceSourceCrashedException();
        }
    }
        ...
        while (true) {
            synchronized (s) {
                if (sharedObject != null) {
                    ...
                    soLogger.info("SATIN '" + s.ident + "': received shared object");
                    return true;
                }
                if (s.currentVictimCrashed) {
                    s.currentVictimCrashed = false;
                    // the source has crashed, abort the job
                    soLogger.info("SATIN '" + s.ident + "': source crashed while waiting for SO reply");
                    throw new SOReferenceSourceCrashedException();
                }
                if (receivedNack) {
                    receivedNack = false;
                    s.currentVictimCrashed = false;
                    soLogger.info("SATIN '" + s.ident + "': received shared object NACK");
                    return false;
                }
    ...
    boolean broadcastInProgress(SharedObjectInfo info, IbisIdentifier dest) {
        ...
        for (IbisIdentifier destination : info.destinations) {
    ...
    protected void handleSORequests() {
        ...
        while (true) {
            ...
            if (v == null) {
                soLogger.debug("SATIN '" + s.ident + "': vicim crached in handleSORequest");
                continue; // node might have crashed
            }

            SharedObjectInfo info = s.so.getSOInfo(objid);
            if (ASSERTS && info == null) {
                soLogger.error("SATIN '" + s.ident + "': EEEK, requested shared object: " + objid + " not found! Exiting..");
                System.exit(1); // Failed assertion
            }

            if (!demand && broadcastInProgress(info, v.getIdent())) {
                soLogger.debug("SATIN '" + s.ident + "': send NACK back in handleSORequest");
                // send NACK back
                try {
                    wm = v.newMessage();
                    ...
                    soLogger.error("SATIN '" + s.ident + "': got exception while sending" + " shared object NACK", e);
                }

                continue;
            }

            soLogger.debug("SATIN '" + s.ident + "': send object back in handleSORequest");
    ...
    private void sendObjectBack(Victim v, SharedObjectInfo info) {
        ...
        try {
            ...
            try {
                ...
                soLogger.error("SATIN '" + s.ident + "': got exception while sending" + " shared object", e);
                return;
            }
    ...
    protected void handleSORequest(ReadMessage m, boolean demand) {
        ...
        try {
            ...
            soLogger.warn("SATIN '" + s.ident + "': got exception while reading" + " shared object request: " + e.getMessage());
        }
    ...
    protected void handleSOTransfer(ReadMessage m) { // normal so transfer (not
        ...
        try {
            ...
            soLogger.error("SATIN '" + s.ident + "': got exception while reading" + " shared object", e);
        } finally {
    ...
    private void connectSOSendPort(IbisIdentifier ident) {
        ...
        ReceivePortIdentifier r = Communication.connect(soSendPort, ident, name, Config.CONNECT_TIMEOUT);
        if (r != null) {
            synchronized (s) {
                ports.put(ident, r);
            }
        } else {
            soLogger.warn("SATIN '" + s.ident + "': unable to connect to SO receive port " + name + " on " + ident);
    ...
    private void connectSendPortToNewReceivers() {
        ...
        for (IbisIdentifier element : tmp) {
            connectSOSendPort(element);
        }
    }
