...
public final class SpawnCounter {
    ...
    /**
     * Obtains a new spawn counter. This does not need to be synchronized, only one
     * thread spawns.
     *
    ...
    static public final void deleteSpawnCounter(SpawnCounter s) {
        if (Config.ASSERTS && s.value < 0) {
            Config.spawnLogger.error("deleteSpawnCounter: spawncouner < 0, val =" + s.value, new Throwable());
    ...
    public void incr(InvocationRecord r) {
        if (Config.ASSERTS && Config.spawnLogger.isDebugEnabled()) {
            debugIncr(r);
        } else {
            value++;
        }
        if (Config.spawnLogger.isDebugEnabled()) {
            Config.spawnLogger.debug("Incremented spawnCounter for " + r.getStamp() + ", value = " + value);
        }
    }
    ...
    private synchronized void debugIncr(InvocationRecord r) {
        ...
        m.put(r, e);
        if (m.size() != value) {
            System.out.println("Incr: hashmap size = " + m.size() + ", value = " + value);
            e.printStackTrace();
        }
    }
    ...
    public void decr(InvocationRecord r) {
        ...
        if (Config.spawnLogger.isDebugEnabled()) {
            Config.spawnLogger.debug("Decremented spawnCounter for " + r.getStamp() + ", value = " + value);
        }
        if (Config.ASSERTS && value < 0) {
    ...
    private synchronized void decrDebug(InvocationRecord r) {
        ...
        if (m.size() != value) {
            System.out.println("Decr: hashmap size = " + m.size() + ", value = " + value);
            lastDecr.printStackTrace();
        }
    }
