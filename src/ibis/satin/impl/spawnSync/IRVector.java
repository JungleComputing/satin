...
public final class IRVector implements Config {
    ...
    public InvocationRecord remove(InvocationRecord r) {
        ...
        // Sometimes (in case of crashes or aborts), we try to remove
        // non-existent elements. This is not a problem, just return null.
        spawnLogger.debug("IRVector: removing non-existent elt: " + r.getStamp());
        return null;
    }
        ...
        for (int i = 0; i < count; i++) {
            ...
            if ((curr.getParent() != null && curr.getParent().aborted) || curr.isDescendentOf(targetStamp)) {
                ...
                if (abortLogger.isDebugEnabled()) {
                    abortLogger.debug("found stolen child: " + curr.getStamp() + ", it depends on " + targetStamp);
                }
    ...
    public void killAndStoreSubtreeOf(IbisIdentifier targetOwner) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if ((curr.getParent() != null && curr.getParent().aborted) || curr.isDescendentOf(targetOwner) || curr.getOwner().equals(targetOwner)) {
                ...
                curr.aborted = true;
                if (abortLogger.isDebugEnabled()) {
                    abortLogger.debug("found stolen child: " + curr.getStamp() + ", it depends on " + targetOwner);
                }
                curr.decrSpawnCounter();
                satin.stats.abortedJobs++;
                satin.stats.abortMessages++;
    ...
    /**
     * Used for fault tolerance. Remove all the jobs stolen by targetOwner and put
     * them back in the taskQueue.
     */
    ...
    public void print(java.io.PrintStream out) {
        ...
        for (int i = 0; i < count; i++) {
            out.println("outjobs [" + i + "] = " + l[i] + "," + l[i].getStealer());
        }
