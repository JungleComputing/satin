...
public final class IRStack implements Config {
    ...
    public ArrayList<InvocationRecord> killChildrenOf(Stamp targetStamp, boolean store) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if ((curr.getParent() != null && curr.getParent().aborted) || curr.isDescendentOf(targetStamp)) {
                curr.aborted = true;
                ...
                if (abortLogger.isDebugEnabled()) {
                    abortLogger.debug("found child on the stack: " + curr.getStamp() + ", it depends on " + targetStamp);
                }
            }
        }
    ...
    /**
     * Used for fault tolerance. Kill every job that was spawned on targetOwner or
     * is a child of a job spawned on targetOwner. Store all finished children of
     * the aborted jobs in the global result table.
     */
    public ArrayList<InvocationRecord> killSubtreesOf(IbisIdentifier targetOwner) {
        Satin.assertLocked(s);
        ArrayList<InvocationRecord> toStore = new ArrayList<>();
        for (int i = 0; i < count; i++) {
            InvocationRecord curr = l[i];
            ...
            if ((curr.getParent() != null && curr.getParent().aborted) || curr.isDescendentOf(targetOwner) || curr.getOwner().equals(targetOwner)) {
    ...
    public ArrayList<InvocationRecord> getAllFinishedChildren(Victim victim) {
        ...
        ArrayList<InvocationRecord> toPush = new ArrayList<>();
    ...
    public ArrayList<ReturnRecord> peekFinishedJobs() {
        ...
        ArrayList<ReturnRecord> result = new ArrayList<>();
        InvocationRecord curr, child;
