...
public final class Statistics implements java.io.Serializable, Config {
    ...
    public void add(Statistics s) {
        ...
        totalCheckpointTime += (s.requestCheckpointTime + s.makeCheckpointTime + s.writeCheckpointTime + s.useCheckpointTime + s.receiveCheckpointTime
                + s.createCoordinatorTime);
    ...
    protected void printStats(int size, double totalTime) {
        ...
        boolean haveCrashes = tableResultUpdates > 0 || tableLookups > 0 || restartedJobs > 0;
        boolean haveSO = soInvocations > 0 || soTransfers > 0 || soBcasts > 0;

        out.println("-------------------------------SATIN STATISTICS------" + "--------------------------");
        out.println("SATIN: SPAWN:       " + nf.format(spawns) + " spawns, " + nf.format(jobsExecuted) + " executed, " + nf.format(syncs) + " syncs");
        if (haveAborts) {
            out.println("SATIN: ABORT:       " + nf.format(abortsDone) + " aborts, " + nf.format(abortMessages) + " abort msgs, "
                    + nf.format(abortedJobs) + " aborted jobs");
        }
        if (haveSteals) {
            out.println("SATIN: STEAL:       " + nf.format(stealAttempts) + " attempts, " + nf.format(stealSuccess) + " successes ("
                    + pf.format(perStats(stealSuccess, stealAttempts) * 100.0) + " %)");

            if (asyncStealAttempts != 0) {
                out.println("SATIN: ASYNCSTEAL:   " + nf.format(asyncStealAttempts) + " attempts, " + nf.format(asyncStealSuccess) + " successes ("
                        + pf.format(perStats(asyncStealSuccess, asyncStealAttempts) * 100.0) + " %)");
            }

            out.println("SATIN: MESSAGES:    intra " + nf.format(intraClusterMessages) + " msgs, " + nf.format(intraClusterBytes) + " bytes; inter "
                    + nf.format(interClusterMessages) + " msgs, " + nf.format(interClusterBytes) + " bytes");
        }

        if (haveCrashes) {
            out.println("SATIN: GLOBAL_RESULT_TABLE: result updates " + nf.format(tableResultUpdates) + ",update messages "
                    + nf.format(tableUpdateMessages) + ", lock updates " + nf.format(tableLockUpdates) + ",lookups " + nf.format(tableLookups)
                    + ",successful " + nf.format(tableSuccessfulLookups) + ",remote " + nf.format(tableRemoteLookups));
            out.println("SATIN: FAULT_TOLERANCE: killed orphans " + nf.format(killedOrphans));
            out.println("SATIN: FAULT_TOLERANCE: restarted jobs " + nf.format(restartedJobs));
        }

        if (haveSO) {
            out.println("SATIN: SO_CALLS:    " + nf.format(soInvocations) + " invocations, " + nf.format(soInvocationsBytes) + " bytes, "
                    + nf.format(soRealMessageCount) + " messages");
            out.println("SATIN: SO_TRANSFER: " + nf.format(soTransfers) + " transfers, " + nf.format(soTransfersBytes) + " bytes ");
            out.println("SATIN: SO_BCAST:    " + nf.format(soBcasts) + " bcasts, " + nf.format(soBcastBytes) + " bytes ");
            out.println("SATIN: SO_GUARDS:    " + nf.format(soGuards) + " guards executed");
        }
        ...
        if (haveSteals) {
            out.println("SATIN: STEAL_TIME:                 total " + Timer.format(stealTime) + " time/req    "
                    + Timer.format(perStats(stealTime, stealAttempts)));
            out.println("SATIN: HANDLE_STEAL_TIME:          total " + Timer.format(handleStealTime) + " time/handle "
                    + Timer.format(perStats(handleStealTime, stealAttempts)));

            out.println("SATIN: THROTTLE_STEAL_TIME:        total " + Timer.format(throttleStealTime) + " time/req    "
                    + Timer.format(perStats(throttleStealTime, stealThrottles)));

            out.println("SATIN: INV SERIALIZATION_TIME:     total " + Timer.format(invocationRecordWriteTime) + " time/write  "
                    + Timer.format(perStats(invocationRecordWriteTime, stealSuccess)));
            out.println("SATIN: INV DESERIALIZATION_TIME:   total " + Timer.format(invocationRecordReadTime) + " time/read   "
                    + Timer.format(perStats(invocationRecordReadTime, stealSuccess)));
            out.println("SATIN: RET SERIALIZATION_TIME:     total " + Timer.format(returnRecordWriteTime) + " time/write  "
                    + Timer.format(perStats(returnRecordWriteTime, returnRecordWriteCount)));
            out.println("SATIN: RET DESERIALIZATION_TIME:   total " + Timer.format(returnRecordReadTime) + " time/read   "
                    + Timer.format(perStats(returnRecordReadTime, returnRecordReadCount)));
        }

        if (haveAborts) {
            out.println("SATIN: ABORT_TIME:                 total " + Timer.format(abortTime) + " time/abort  "
                    + Timer.format(perStats(abortTime, abortsDone)));
        }

        if (haveCrashes) {
            out.println("SATIN: GRT_UPDATE_TIME:            total " + Timer.format(tableUpdateTime) + " time/update "
                    + Timer.format(perStats(tableUpdateTime, (tableResultUpdates + tableLockUpdates))));
            out.println("SATIN: GRT_LOOKUP_TIME:            total " + Timer.format(tableLookupTime) + " time/lookup "
                    + Timer.format(perStats(tableLookupTime, tableLookups)));
            out.println("SATIN: GRT_HANDLE_UPDATE_TIME:     total " + Timer.format(tableHandleUpdateTime) + " time/handle "
                    + Timer.format(perStats(tableHandleUpdateTime, tableResultUpdates * (size - 1))));
            out.println("SATIN: GRT_HANDLE_LOOKUP_TIME:     total " + Timer.format(tableHandleLookupTime) + " time/handle "
                    + Timer.format(perStats(tableHandleLookupTime, tableRemoteLookups)));
            out.println("SATIN: GRT_SERIALIZATION_TIME:     total " + Timer.format(tableSerializationTime));
            out.println("SATIN: GRT_DESERIALIZATION_TIME:   total " + Timer.format(tableDeserializationTime));
            out.println("SATIN: GRT_CHECK_TIME:             total " + Timer.format(tableCheckTime) + " time/check "
                    + Timer.format(perStats(tableCheckTime, tableLookups)));
            out.println("SATIN: CRASH_HANDLING_TIME:        total " + Timer.format(crashHandlingTime));
        }

        // Checkpointing.
        if (CHECKPOINTING) {
            out.println("SATIN: REQUEST_CHECKPOINT_TIME:    total " + Timer.format(requestCheckpointTime) + " time/checkpoint "
                    + Timer.format(perStats(requestCheckpointTime, numCheckpointsTaken)));
            out.println("SATIN: MAKE_CHECKPOINT_TIME:       total " + Timer.format(makeCheckpointTime) + " time/checkpoint "
                    + Timer.format(perStats(makeCheckpointTime, numCheckpointsTaken)));
            out.println("SATIN: RECEIVE_CHECKPOINT_TIME:    total " + Timer.format(receiveCheckpointTime) + " time/checkpoint "
                    + Timer.format(perStats(receiveCheckpointTime, numCheckpointsTaken)));
            out.println("SATIN: WRITE_CHECKPOINT_TIME:      total " + Timer.format(writeCheckpointTime) + " time/checkpoint "
                    + Timer.format(perStats(writeCheckpointTime, numCheckpointsTaken)));
            out.println("SATIN: USE_CHECKPOINT_TIME:        total " + Timer.format(useCheckpointTime));
            out.println("SATIN: CREATE_COORDINATOR_TIME:    total " + Timer.format(createCoordinatorTime));
            out.println("SATIN: TOTAL_CHECKPOINT_TIME:      total " + Timer.format(totalCheckpointTime));
            out.println("SATIN: NUM_CHECKPOINTS_TAKEN:    " + nf.format(numCheckpointsTaken));
        }

        if (haveSO) {
            out.println("SATIN: BROADCAST_SO_INVOCATIONS:   total " + Timer.format(broadcastSOInvocationsTime) + " time/inv    "
                    + Timer.format(perStats(broadcastSOInvocationsTime, soInvocations)));
            out.println("SATIN: DESERIALIZE_SO_INVOCATIONS: total " + Timer.format(soInvocationDeserializationTime) + " time/inv    "
                    + Timer.format(perStats(soInvocationDeserializationTime, handleSOInvocations)));
            out.println("SATIN: HANDLE_SO_INVOCATIONS:      total " + Timer.format(handleSOInvocationsTime) + " time/inv    "
                    + Timer.format(perStats(handleSOInvocationsTime, handleSOInvocations)));
            out.println("SATIN: GET_SO_REFERENCES:          total " + Timer.format(getSOReferencesTime) + " time/inv    "
                    + Timer.format(perStats(getSOReferencesTime, getSOReferences)));
            out.println("SATIN: SO_TRANSFERS:               total " + Timer.format(soTransferTime) + " time/transf "
                    + Timer.format(perStats(soTransferTime, soTransfers)));
            out.println("SATIN: SO_SERIALIZATION:           total " + Timer.format(soSerializationTime) + " time/transf "
                    + Timer.format(perStats(soSerializationTime, soTransfers)));
            out.println("SATIN: SO_DESERIALIZATION:         total " + Timer.format(soDeserializationTime) + " time/transf "
                    + Timer.format(perStats(soDeserializationTime, soTransfers)));
            out.println("SATIN: SO_BCASTS:                  total " + Timer.format(soBcastTime) + " time/bcast  "
                    + Timer.format(perStats(soBcastTime, soBcasts)));
            out.println("SATIN: SO_BCAST_SERIALIZATION:     total " + Timer.format(soBcastSerializationTime) + " time/bcast  "
                    + Timer.format(perStats(soBcastSerializationTime, soBcasts)));
            out.println("SATIN: SO_BCAST_DESERIALIZATION:   total " + Timer.format(soBcastDeserializationTime) + " time/bcast  "
                    + Timer.format(perStats(soBcastDeserializationTime, soBcasts)));
            out.println("SATIN: SO_GUARDS:                  total " + Timer.format(soGuardTime) + " time/guard  "
                    + Timer.format(perStats(soGuardTime, soGuards)));
        }

        out.println("-------------------------------SATIN RUN TIME " + "BREAKDOWN------------------------");
        out.println("SATIN: TOTAL_RUN_TIME:                              " + Timer.format(totalTime));

        double lbTime = (stealTime + throttleStealTime - invocationRecordReadTime - invocationRecordWriteTime - returnRecordReadTime
                - returnRecordWriteTime) / size;
        if (lbTime < 0.0) {
            lbTime = 0.0;
        }
        ...
        double serTimeAvg = (invocationRecordWriteTime + invocationRecordReadTime + returnRecordWriteTime + returnRecordReadTime) / size;
        ...
        double tableHandleUpdatePerc = tableHandleUpdateTimeAvg / totalTime * 100.0;
        double tableHandleLookupTimeAvg = tableHandleLookupTime / size;
        double tableHandleLookupPerc = tableHandleLookupTimeAvg / totalTime * 100.0;
        double tableSerializationTimeAvg = tableSerializationTime / size;
        double tableSerializationPerc = tableSerializationTimeAvg / totalTime * 100;
        double tableDeserializationTimeAvg = tableDeserializationTime / size;
        double tableDeserializationPerc = tableDeserializationTimeAvg / totalTime * 100;
        ...
        double broadcastSOInvocationsTimeAvg = broadcastSOInvocationsTime / size;
        double broadcastSOInvocationsPerc = broadcastSOInvocationsTimeAvg / totalTime * 100;
        double handleSOInvocationsTimeAvg = handleSOInvocationsTime / size;
        double handleSOInvocationsPerc = handleSOInvocationsTimeAvg / totalTime * 100;
        double soInvocationDeserializationTimeAvg = soInvocationDeserializationTime / size;
        double soInvocationDeserializationPerc = soInvocationDeserializationTimeAvg / totalTime * 100;
        ...
        double soDeserializationPerc = soDeserializationTimeAvg / totalTime * 100;
        ...
        double soBcastSerializationPerc = soBcastSerializationTimeAvg / totalTime * 100;
        double soBcastDeserializationTimeAvg = soBcastDeserializationTime / size;
        double soBcastDeserializationPerc = soBcastDeserializationTimeAvg / totalTime * 100;
        double soGuardTimeAvg = soGuardTime / size;
        double soGuardPerc = soGuardTimeAvg / totalTime * 100;

        double totalOverheadAvg = abortTimeAvg + tableUpdateTimeAvg + tableLookupTimeAvg + tableHandleUpdateTimeAvg + tableHandleLookupTimeAvg
                + handleSOInvocationsTimeAvg + broadcastSOInvocationsTimeAvg + soTransferTimeAvg + soBcastTimeAvg + soBcastDeserializationTimeAvg
                + stealTimeAvg + throttleTimeAvg + soGuardTimeAvg;
        ...
        if (haveSteals) {
            out.println("SATIN: STEAL_THROTTLE_TIME:        agv. per machine " + Timer.format(throttleTimeAvg) + " (" + (throttlePerc < 10 ? " " : "")
                    + pf.format(throttlePerc) + " %)");
            out.println("SATIN: (DE)SERIALIZATION_TIME:     agv. per machine " + Timer.format(serTimeAvg) + " (" + (serPerc < 10 ? " " : "")
                    + pf.format(serPerc) + " %)");
            out.println("SATIN: LOAD_BALANCING_TIME:        agv. per machine " + Timer.format(lbTime) + " (" + (lbPerc < 10 ? " " : "")
                    + pf.format(lbPerc) + " %)");
        }

        if (haveAborts) {
            out.println("SATIN: ABORT_TIME:                 agv. per machine " + Timer.format(abortTimeAvg) + " (" + (abortPerc < 10 ? " " : "")
                    + pf.format(abortPerc) + " %)");
        }

        if (haveCrashes) {
            out.println("SATIN: GRT_UPDATE_TIME:            agv. per machine " + Timer.format(tableUpdateTimeAvg) + " (" + pf.format(tableUpdatePerc)
                    + " %)");
            out.println("SATIN: GRT_LOOKUP_TIME:            agv. per machine " + Timer.format(tableLookupTimeAvg) + " (" + pf.format(tableLookupPerc)
                    + " %)");
            out.println("SATIN: GRT_HANDLE_UPDATE_TIME:     agv. per machine " + Timer.format(tableHandleUpdateTimeAvg) + " ("
                    + pf.format(tableHandleUpdatePerc) + " %)");
            out.println("SATIN: GRT_HANDLE_LOOKUP_TIME:     agv. per machine " + Timer.format(tableHandleLookupTimeAvg) + " ("
                    + pf.format(tableHandleLookupPerc) + " %)");
            out.println("SATIN: GRT_SERIALIZATION_TIME:     agv. per machine " + Timer.format(tableSerializationTimeAvg) + " ("
                    + pf.format(tableSerializationPerc) + " %)");
            out.println("SATIN: GRT_DESERIALIZATION_TIME:   agv. per machine " + Timer.format(tableDeserializationTimeAvg) + " ("
                    + pf.format(tableDeserializationPerc) + " %)");
            out.println("SATIN: CRASH_HANDLING_TIME:        agv. per machine " + Timer.format(crashHandlingTimeAvg) + " ("
                    + pf.format(crashHandlingPerc) + " %)");
        }

        if (CHECKPOINTING) {
            out.println("SATIN: REQUEST_CHECKPOINT_TIME:    avg. per machine " + Timer.format(requestCheckpointTimeAvg) + " ("
                    + pf.format(requestCheckpointPerc) + " %)");
            out.println("SATIN: MAKE_CHECKPOINT_TIME:       avg. per machine " + Timer.format(makeCheckpointTimeAvg) + " ("
                    + pf.format(makeCheckpointPerc) + " %)");
            out.println("SATIN: RECEIVE_CHECKPOINT_TIME:    avg. per machine " + Timer.format(receiveCheckpointTimeAvg) + " ("
                    + pf.format(receiveCheckpointPerc) + " %)");
            out.println("SATIN: WRITE_CHECKPOINT_TIME:      avg. per machine " + Timer.format(writeCheckpointTimeAvg) + " ("
                    + pf.format(writeCheckpointPerc) + " %)");
            out.println("SATIN: USE_CHECKPOINT_TIME:        avg. per machine " + Timer.format(useCheckpointTimeAvg) + " ("
                    + pf.format(useCheckpointPerc) + " %)");
            out.println("SATIN: CREATE_COORDINATOR_TIME:    avg. per machine " + Timer.format(createCoordinatorTimeAvg) + " ("
                    + pf.format(createCoordinatorPerc) + " %)");
            out.println("SATIN: TOTAL_CHECKPOINT_TIME:      avg. per machine " + Timer.format(totalCheckpointTimeAvg) + " ("
                    + pf.format(totalCheckpointPerc) + " %)");
        }

        if (haveSO) {
            out.println("SATIN: BROADCAST_SO_INVOCATIONS:   agv. per machine " + Timer.format(broadcastSOInvocationsTimeAvg) + " ("
                    + pf.format(broadcastSOInvocationsPerc) + " %)");
            out.println("SATIN: HANDLE_SO_INVOCATIONS:      agv. per machine " + Timer.format(handleSOInvocationsTimeAvg) + " ("
                    + pf.format(handleSOInvocationsPerc) + " %)");
            out.println("SATIN: DESERIALIZE_SO_INVOCATIONS: agv. per machine " + Timer.format(soInvocationDeserializationTimeAvg) + " ( "
                    + pf.format(soInvocationDeserializationPerc) + " %)");
            out.println("SATIN: SO_TRANSFERS:               agv. per machine " + Timer.format(soTransferTimeAvg) + " (" + pf.format(soTransferPerc)
                    + " %)");
            out.println("SATIN: SO_SERIALIZATION:           agv. per machine " + Timer.format(soSerializationTimeAvg) + " ("
                    + pf.format(soSerializationPerc) + " %)");
            out.println("SATIN: SO_DESERIALIZATION:         agv. per machine " + Timer.format(soDeserializationTimeAvg) + " ("
                    + pf.format(soDeserializationPerc) + " %)");
            out.println(
                    "SATIN: SO_BCASTS:                  agv. per machine " + Timer.format(soBcastTimeAvg) + " (" + pf.format(soBcastPerc) + " %)");
            out.println("SATIN: SO_BCAST_SERIALIZATION:     agv. per machine " + Timer.format(soBcastSerializationTimeAvg) + " ("
                    + pf.format(soBcastSerializationPerc) + " %)");
            out.println("SATIN: SO_BCAST_DESERIALIZATION:   agv. per machine " + Timer.format(soBcastDeserializationTimeAvg) + " ("
                    + pf.format(soBcastDeserializationPerc) + " %)");
            out.println(
                    "SATIN: SO_GUARD:                   agv. per machine " + Timer.format(soGuardTimeAvg) + " (" + pf.format(soGuardPerc) + " %)");
        }

        out.println("\nSATIN: TOTAL_PARALLEL_OVERHEAD:    agv. per machine " + Timer.format(totalOverheadAvg) + " (" + (totalPerc < 10 ? " " : "")
                + pf.format(totalPerc) + " %)");

        out.println("SATIN: USEFUL_APP_TIME:            agv. per machine " + Timer.format(appTime) + " (" + (appPerc < 10 ? " " : "")
                + pf.format(appPerc) + " %)");
    }
    ...
    public void printDetailedStats(IbisIdentifier ident) {
        ...
        out.println("SATIN '" + ident + "': SPAWN_STATS: spawns = " + spawns + " executed = " + jobsExecuted + " syncs = " + syncs + " aborts = "
                + abortsDone + " abort msgs = " + abortMessages + " aborted jobs = " + abortedJobs + " total time = " + abortTimer.totalTime());

        out.println("SATIN '" + ident + "': MSG_STATS: intra = " + intraClusterMessages + ", bytes = " + nf.format(intraClusterBytes) + " inter = "
                + interClusterMessages + ", bytes = " + nf.format(interClusterBytes));

        out.println("SATIN '" + ident + "': STEAL_STATS: attempts = " + stealAttempts + " success = " + stealSuccess + " ("
                + (perStats(stealSuccess, stealAttempts) * 100.0) + " %)" + " time = " + stealTimer.totalTime() + " requests = " + stealRequests
                + " jobs stolen = " + stolenJobs + " time = " + handleStealTimer.totalTime());
        out.println("SATIN '" + ident + "': ASYNCSTEAL_STATS: attempts = " + asyncStealAttempts + " success = " + asyncStealSuccess + " ("
                + (perStats(asyncStealSuccess, asyncStealAttempts) * 100.0) + " %)");

        out.println("SATIN '" + ident + "': STEAL_THROTTLE_STATS: throttles = " + stealThrottleTimer.nrTimes() + " time = "
                + stealThrottleTimer.totalTime());

        out.println("SATIN '" + ident + "': SERIALIZATION_STATS: invocationRecordWrites = " + invocationRecordWriteTimer.nrTimes() + " total time = "
                + invocationRecordWriteTimer.totalTime() + " invocationRecordReads = " + invocationRecordReadTimer.nrTimes() + " total time = "
                + invocationRecordReadTimer.totalTime() + " returnRecordWrites = " + returnRecordWriteTimer.nrTimes() + " total time = "
                + returnRecordWriteTimer.totalTime() + " returnRecordReads = " + returnRecordReadTimer.nrTimes() + " total time = "
                + returnRecordReadTimer.totalTime());

        out.println("SATIN '" + ident + "': GRT_STATS: updates = " + tableResultUpdates + " lock updates = " + tableLockUpdates + " update time = "
                + updateTimer.totalTime() + " handle update time = " + handleUpdateTimer.totalTime() + " msgs = " + tableUpdateMessages
                + " lookups = " + tableLookups + " lookup time = " + lookupTimer.totalTime() + " handle lookup time = " + lookupTimer.totalTime()
                + " remote lookups = " + tableRemoteLookups + " successful lookups = " + tableSuccessfulLookups + " max size = " + tableMaxEntries);

        out.println("SATIN '" + ident + "': FT_STATS:" + " handle crash time = " + crashTimer.totalTime() + " redo time = " + redoTimer.totalTime()
                + " orphans killed = " + killedOrphans + " jobs restarted = " + restartedJobs);

        out.println("SATIN '" + ident + "': SO_STATS: " + " invocations = " + soInvocations + " size = " + soInvocationsBytes + " time = "
                + broadcastSOInvocationsTimer.totalTime() + " transfers = " + soTransfers + " size = " + soTransfersBytes + " time = "
