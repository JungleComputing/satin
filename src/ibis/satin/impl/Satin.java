...
public final class Satin implements Config {
    ...
    /**
     * This vector contains all jobs that were stolen from me. Used to locate the
     * invocation record corresponding to the result of a remote job.
     */
    ...
    /**
     * Used for fault tolerance. All ibises that once took part in the computation,
     * but then crashed. Assumption: ibis identifiers are uniqe in time; the same
     * ibis cannot crash and join the computation again.
     */
    public final Vector<IbisIdentifier> deadIbises = new Vector<>();
     ...
     * constructor gets called by the rewritten main() from the application, and the
     * argument array from main is passed to this constructor. Which ibis is chosen
     * depends, a.o., on these arguments.
     */
    public Satin() {
        if (thisSatin != null) {
    ...
    /**
     * Called at the end of the rewritten main in case the original main threw an
     * exception.
     */
     ...
     * The invocation record is added to the job queue maintained by this Satin.
     *
    ...
    public void sync(SpawnCounter s) {
        ...
        } else { // A sync without spawns is a poll.
            while (s.getValue() > 0) {
    ...
    /**
     * Aborts the spawns that are the result of the specified invocation record. The
     * invocation record of the invocation actually throwing the exception is also
     * specified, but it is valid only for clones with inlets.
     *
    ...
    /**
     * Pause Satin operation. This method can optionally be called before a large
     * sequential part in a program. This will temporarily pause Satin's internal
     * load distribution strategies to avoid communication overhead during
     * sequential code.
     */
    ...
    /**
     * Resume Satin operation. This method can optionally be called after a large
     * sequential part in a program.
     */
    ...
    /**
     * Returns whether it might be useful to spawn more methods. If there is enough
     * work in the system to keep all processors busy, this method returns false.
     */
    public static boolean needMoreJobs() {
        ...
        synchronized (thisSatin) {
            ...
            if ((size == 0 && CLOSED) || (thisSatin.q.size() / (size + 1) > SUGGESTED_QUEUE_SIZE)) {
                return false;
            }
        }
    ...
    /**
     * Returns whether the current method was generated by the machine it is running
     * on. Methods can be distributed to remote machines by the Satin runtime
     * system, in which case this method returns false.
     */
    public static boolean localJob() {
        if ((thisSatin == null) || (thisSatin.parent == null)) {
    ...
    private void callSatinRemoteFunction(InvocationRecord r) {
        ...
        } else if (stealLogger.isInfoEnabled()) {
