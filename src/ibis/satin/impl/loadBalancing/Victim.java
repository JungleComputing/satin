...
public final class Victim implements Config {
    ...
    private void disconnect() throws IOException {
        if (connected) {
            ...
            if (commLogger.isDebugEnabled()) {
                commLogger.debug("SATIN '" + sendPort.identifier().ibisIdentifier() + "': disconnected from " + ident);
            }
        }
    }
    ...
    private SendPort getSendPort() {
        ...
        if (!connected) {
            if (commLogger.isDebugEnabled()) {
                commLogger.debug("SATIN '" + sendPort.identifier().ibisIdentifier() + "': connecting to " + ident);
            }
            r = Communication.connect(sendPort, ident, "satin port", Config.CONNECT_TIMEOUT);
            if (r == null) {
                commLogger.warn("SATIN '" + sendPort.identifier().ibisIdentifier() + "': unable to connect to " + ident + ", might have crashed");
                return null;
            }
            connected = true;
            connectionCount++;
    ...
    public WriteMessage newMessage() throws IOException {
        ...
        synchronized (sendPort) {
            ...
            if (send != null) {
                ...
                throw new IOException("SATIN '" + sendPort.identifier().ibisIdentifier() + "': Could not connect to " + ident);
            }
        }
    ...
    public void close() {
        synchronized (sendPort) {
            ...
            try {
                ...
                // ignore
                Config.commLogger
                        .warn("SATIN '" + sendPort.identifier().ibisIdentifier() + "': port.close() throws exception (ignored), dest = " + ident, e);
            }
        }
    }
