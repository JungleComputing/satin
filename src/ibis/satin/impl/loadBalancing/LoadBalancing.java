...
public final class LoadBalancing implements Config {
    ...
    /**
     * Used for fault tolerance, we must know who the current victim is, in case it
     * crashes.
     */
    ...
    public synchronized void gotJobResult(InvocationRecord ir, IbisIdentifier sender) {
        ...
        if (!sender.equals(currentVictim)) {
            if (s.deadIbises.contains(sender)) {
                ...
                ftLogger.warn("SATIN '" + s.ident + "': received a reply from " + sender + " which is supposed to be dead");
                return;
            }
            ftLogger.warn(
                    "SATIN '" + s.ident + "': received a reply from " + sender + " who caused a timeout before. I am stealing from " + currentVictim);
            if (ir != null) {
                s.q.addToTail(ir);
            }
            return;
    ...
    /**
     * does a synchronous steal. If blockOnServer is true, it blocks on server side
     * until work is available, or we must exit. This is used in MasterWorker
     * algorithms.
     */
    public InvocationRecord stealJob(Victim v, boolean blockOnServer) {
        if (ASSERTS) {
            synchronized (this) {
                if (stolenJob != null) {
                    ...
                    throw new Error("EEEK, trying to steal while an unhandled stolen job is available.");
                }
            }
        }
        ...
        if (Satin.GLOBAL_PAUSE_RESUME) {
            synchronized (s) {
                if (s.comm.paused) {
                    ...
                    commLogger.info("SATIN '" + s.ident + "': paused for " + (end - start) + " ms");
                }
            }
        }
        ...
        try {
            ...
            ftLogger.info("SATIN '" + s.ident + "': got exception during steal request", e);
            return null;
        } finally {
            ...
            synchronized (this) {
                ...
                if (gotTimeout && !gotStealReply) {
                    if (!("MW".equals(SUPPLIED_ALG))) {
                        ftLogger.warn("SATIN '" + s.ident + "': a timeout occurred while waiting for a steal reply from victim " + v.getIdent()
                                + ", timeout = " + STEAL_WAIT_TIMEOUT / 1000 + " seconds.");
                    }
                }
                ...
                if (s.currentVictimCrashed) {
                    s.currentVictimCrashed = false;
                    ftLogger.debug("SATIN '" + s.ident + "': current victim crashed");
                    return;
                }
    ...
    protected void addJobResult(ReturnRecord rr, Throwable eek, Stamp stamp) {
        synchronized (s) {
            ...
            if (r != null) {
                ...
                abortLogger.debug("SATIN '" + s.ident + "': got result for aborted job, ignoring.");
            }
        }
    }

    // throws an IO exception when the ibis that tried to steal the job dies
    protected InvocationRecord stealJobFromLocalQueue(SendPortIdentifier ident, boolean blocking) throws IOException {
    ...
    public void sendStealRequest(Victim v, boolean synchronous, boolean blocking) throws IOException {
