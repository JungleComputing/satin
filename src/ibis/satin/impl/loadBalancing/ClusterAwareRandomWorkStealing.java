...
public final class ClusterAwareRandomWorkStealing extends LoadBalancingAlgorithm implements Protocol {
        ...
        if (remoteVictim != null && !asyncStealInProgress) {
            if (FT_NAIVE || canDoAsync) {
                ...
                try {
                    ...
                    commLogger.warn("SATIN '" + s.ident + "': Got exception during wa steal request: " + e);
                    // Ignore this?
                }
            }
        }
    ...
    private InvocationRecord checkForAsyncReply() {
        ...
        synchronized (satin) {
            ...
            if (gotTimeout && !gotAsyncStealReply) {
                ftLogger.warn("SATIN '" + s.ident + "': a timeout occurred while waiting for a wide-area steal reply from " + asyncCurrentVictim
        ...
        case ASYNC_STEAL_REPLY_FAILED_TABLE:
            ...
            synchronized (satin) {
                if (sender.equals(asyncCurrentVictim)) {
                    ...
                    ftLogger.warn("SATIN '" + s.ident + "': received an async job from a node that caused a timeout before.");
                    if (ir != null) {
                        s.q.addToTail(ir);
                    }
                }
            ...
            break;
        default:
            s.assertFailed("illegal opcode in CRS stealReplyHandler", new Exception());
        }
    }
    ...
    public void exit() {
        ...
        if (asyncStealInProgress) {
            stealLogger.info("waiting for a pending async steal reply from " + asyncCurrentVictim);
            synchronized (satin) {
            ...
            if (ASSERTS && asyncStolenJob != null) {
                stealLogger.warn("Satin: CRS: EEK, stole async job " + "after exiting!");
            }
        }
    }

    /**
     * Used in fault tolerance; check if the asynchronous steal victim crashed; if
     * so, cancel the steal request; if the job already arrived, remove it (it
     * should be aborted anyway, since it was stolen from a crashed machine) if the
     * owner of the asynchronously stolen job crashed, abort the job.
     */
    @Override
