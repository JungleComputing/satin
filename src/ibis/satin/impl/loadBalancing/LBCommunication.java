...
final class LBCommunication implements Config, Protocol {
    ...
    protected void sendStealRequest(Victim v, boolean synchronous, boolean blockUntilWorkIsAvailable) throws IOException {
        if (stealLogger.isDebugEnabled()) {
            stealLogger.debug("SATIN '" + s.ident + "': sending " + (synchronous ? "SYNC" : "ASYNC") + "steal message to " + v.getIdent());
        }
                ...
                if (s.clusterCoordinator && s.ft.getTable) {
                    ...
                    if (s.ft.getTable) {
                        if (grtLogger.isInfoEnabled()) {
                            grtLogger.info("SATIN '" + s.ident + ": EEEK sending async steal message " + "while waiting for table!!");
                        }
                    }
    ...
    protected void handleJobResult(ReadMessage m, int opcode) {
        ...
        if (stealLogger.isInfoEnabled()) {
            stealLogger.info("SATIN '" + s.ident + "': got job result message from " + m.origin().ibisIdentifier());
        }
        ...
        try {
            ...
            spawnLogger.error("SATIN '" + s.ident + "': got exception while reading job result: " + e + opcode, e);
            gotException = true;
        } finally {
        ...
        if (stealLogger.isInfoEnabled()) {
            if (eek != null) {
                stealLogger.info("SATIN '" + s.ident + "': handleJobResult: exception result: " + eek + ", stamp = " + stamp, eek);
            } else {
                stealLogger.info("SATIN '" + s.ident + "': handleJobResult: normal result, stamp = " + stamp);
            }
        }
    ...
    protected void sendResult(InvocationRecord r, ReturnRecord rr) {
        ...
        if (stealLogger.isInfoEnabled()) {
            stealLogger.info("SATIN '" + s.ident + "': sending job result to " + r.getOwner() + ", exception = "
                    + (r.eek == null ? "null" : ("" + r.eek)) + ", stamp = " + r.getStamp());
        }
        ...
        synchronized (s) {
            if (!FT_NAIVE && r.isOrphan()) {
                ...
                if (ASSERTS && owner == null) {
                    grtLogger.error("SATIN '" + s.ident + "': orphan not locked in the table");
        ...
        if (v == null) {
            ...
            if (!FT_NAIVE && !r.isOrphan()) {
                ...
                if (grtLogger.isInfoEnabled()) {
                    grtLogger.info("SATIN '" + s.ident + "': a job became an orphan??");
                }
            }
            return;
        }
        ...
        try {
            ...
            if (e instanceof NotSerializableException || e instanceof InvalidClassException) {
                ftLogger.warn("SATIN '" + s.ident + "': Got exception while sending result of stolen job", e);
            } else if (ftLogger.isInfoEnabled()) {
                ftLogger.info("SATIN '" + s.ident + "': Got exception while sending result of stolen job", e);
            }
        } finally {
    ...
    protected void handleStealRequest(SendPortIdentifier ident, int opcode) {
        ...
        try {
            ...
            if (stealLogger.isDebugEnabled()) {
                stealLogger.debug("SATIN '" + s.ident + "': dealing with steal request from " + ident.ibisIdentifier() + " opcode = "
            ...
            synchronized (s) {
                ...
                if (v == null || s.deadIbises.contains(ident.ibisIdentifier())) {
                    ...
                    stealLogger.warn("SATIN '" + s.ident + "': EEK!! got steal request from a dead ibis: " + ident.ibisIdentifier());
                    return;
                }

                try {
                    result = lb.stealJobFromLocalQueue(ident, opcode == BLOCKING_STEAL_REQUEST);
                } catch (IOException e) {
                    stealLogger.warn("SATIN '" + s.ident + "': EEK!! got exception during steal request: " + ident.ibisIdentifier());
                    return; // the stealing ibis died
                }

                if (!FT_NAIVE && (opcode == STEAL_AND_TABLE_REQUEST || opcode == ASYNC_STEAL_AND_TABLE_REQUEST)) {
                    if (!s.ft.getTable) {
    ...
    protected void handleReply(ReadMessage m, int opcode) {
        ...
        if (stealLogger.isDebugEnabled()) {
            stealLogger.debug(
                    "SATIN '" + s.ident + "': got steal reply message from " + ident.ibisIdentifier() + ": " + Communication.opcodeToString(opcode));
        }
        ...
        case ASYNC_STEAL_REPLY_SUCCESS:
            try {
                ...
                if (ASSERTS && tmp.aborted) {
                    stealLogger.warn("SATIN '" + s.ident + ": stole aborted job!");
                }
            } catch (Exception e) {
                stealLogger.error("SATIN '" + s.ident + "': Got Exception while reading steal " + "reply from " + ident + ", opcode:" + opcode
                        + ", exception: " + e, e);
            } finally {
            ...
            synchronized (s) {
                if (s.deadIbises.contains(ident)) {
                    ...
                    stealLogger.error("SATIN '" + s.ident + "': got reply from dead ibis??? Ignored");
                    break;
                }
            }
    ...
    private void sendStealFailedMessage(SendPortIdentifier ident, int opcode, Victim v, Map<Stamp, GlobalResultTableValue> table) {

        if (stealLogger.isDebugEnabled()) {
            if (opcode == ASYNC_STEAL_REQUEST) {
                stealLogger.debug("SATIN '" + s.ident + "': sending FAILED back to " + ident.ibisIdentifier());
            }
            if (opcode == ASYNC_STEAL_AND_TABLE_REQUEST) {
                stealLogger.debug("SATIN '" + s.ident + "': sending FAILED_TABLE back to " + ident.ibisIdentifier());
            }
        }
        ...
        try {
            ...
            if (opcode == STEAL_REQUEST || opcode == BLOCKING_STEAL_REQUEST) {
                ...
                stealLogger.error("UNHANDLED opcode " + opcode + " in handleStealRequest");
            }

            v.finish(m);

            if (stealLogger.isDebugEnabled()) {
                stealLogger.debug("SATIN '" + s.ident + "': sending FAILED back to " + ident.ibisIdentifier() + " DONE");
            }
        } catch (IOException e) {
            if (m != null) {
                m.finish(e);
            }
            stealLogger.warn("SATIN '" + s.ident + "': trying to send FAILURE back, but got exception: " + e, e);
        }
    }

    private void sendStolenJobMessage(SendPortIdentifier ident, int opcode, Victim v, InvocationRecord result,
            Map<Stamp, GlobalResultTableValue> table) {
        if (ASSERTS && result.aborted) {
            stealLogger.warn("SATIN '" + s.ident + ": trying to send aborted job!");
        }

        s.stats.stolenJobs++;

        if (stealLogger.isInfoEnabled()) {
            stealLogger.info("SATIN '" + s.ident + "': sending SUCCESS and job #" + result.getStamp() + " back to " + ident.ibisIdentifier());
        }
        ...
        try {
            ...
            if (opcode == STEAL_REQUEST || opcode == BLOCKING_STEAL_REQUEST) {
                ...
                if (table != null) {
                    ...
                    stealLogger.warn("SATIN '" + s.ident + "': EEK!! sending a job but not a table !?");
                }
            } else if (opcode == ASYNC_STEAL_AND_TABLE_REQUEST) {
                if (table != null) {
            ...
            stealLogger.warn("SATIN '" + s.ident + "': trying to send a job back, but got exception: " + e, e);
        }

        /*
         * If we don't use fault tolerance with the global result table, we can set the
         * object parameters to null, so the GC can clean them up --Rob
         */
