...
public abstract class LoadBalancingAlgorithm implements Config {
    ...
    /**
     * Called in every iteration of the client loop. It decides which jobs are run,
     * and what kind(s) of steal requests are done. returns a job an success, null
     * on failure.
     */
    ...
    /**
     * This one is called in the exit procedure so the algorithm can clean up, e.g.,
     * wait for pending (async) messages Default implementation does nothing.
     */
    public void exit() {
        synchronized (satin) {
            satin.notifyAll();
