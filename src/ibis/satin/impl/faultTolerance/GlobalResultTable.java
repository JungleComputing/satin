...
public final class GlobalResultTable implements Config, Protocol {
    ...
    private GlobalResultTableValue pointerValue = new GlobalResultTableValue(GlobalResultTableValue.TYPE_POINTER);
        ...
        try {
            ...
            if (value != null) {
                if (grtLogger.isDebugEnabled()) {
                    grtLogger.debug("SATIN '" + s.ident + "': lookup successful " + key);
                }
            ...
            } else if (grtLogger.isDebugEnabled()) {
                grtLogger.debug("SATIN '" + s.ident + "': lookup failed " + key);
            }
    ...
    private void update(Stamp key, GlobalResultTableValue value) {
        ...
        try {
            ...
            grtLogger.debug("SATIN '" + s.ident + "': update complete: " + key + "," + value);
    ...
    public void storeResult(InvocationRecord r) {
        update(r.getStamp(), new GlobalResultTableValue(GlobalResultTableValue.TYPE_RESULT, r));
    }
        ...
        try {
            ...
            if (grtLogger.isDebugEnabled()) {
                for (Stamp key : updates.keySet()) {
                    grtLogger.debug("SATIN '" + s.ident + "': update: " + key + "," + updates.get(key));
                }
            }
        } finally {
            s.stats.updateTimer.stop();
            ...
            for (int i = 0; i < size; i++) {
                ...
                try {
                    ...
                    if (e instanceof NotSerializableException || e instanceof InvalidClassException) {
                        grtLogger.warn("SATIN '" + s.ident + "': Got exception in writeObject()", e);
                ...
                try {
                    ...
                    grtLogger.debug("SATIN '" + s.ident + "': " + msgSize + " sent in " + s.stats.tableSerializationTimer.lastTimeVal() + " to " + v);
                } catch (IOException e) {
                    m.finish(e);
    ...
    protected Map<Stamp, GlobalResultTableValue> getContents() {
        ...
        Map<Stamp, GlobalResultTableValue> newEntries = new HashMap<>();
        ...
        for (Map.Entry<Stamp, GlobalResultTableValue> entry : entries.entrySet()) {
            ...
            case GlobalResultTableValue.TYPE_POINTER:
                ...
                break;
            default:
                grtLogger.error("SATIN '" + s.ident + "': EEK invalid value type in getContents()");
            }
        }
    ...
    protected void handleGRTUpdate(ReadMessage m) {
        ...
        try {
            ...
            Map<Stamp, GlobalResultTableValue> map = (Map<Stamp, GlobalResultTableValue>) m.readObject();
            synchronized (s) {
                addContents(map);
            }
        } catch (Exception e) {
            grtLogger.error("SATIN '" + s.ident + "': Global result table - error reading message", e);
        } finally {
    ...
    protected void print(java.io.PrintStream out) {
        synchronized (s) {
            ...
            for (Map.Entry<Stamp, GlobalResultTableValue> entry : entries.entrySet()) {
                out.println("GRT[" + i + "]= " + entry.getKey() + ";" + entry.getValue());
                i++;
            }
