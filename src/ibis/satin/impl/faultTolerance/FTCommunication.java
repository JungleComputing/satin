...
final class FTCommunication implements Config, ReceivePortConnectUpcall, SendPortDisconnectUpcall, RegistryEventHandler {
    private Satin s;
    ...
    protected void electClusterCoordinator() {
        try {
            String election = "satin " + Victim.clusterOf(s.comm.ibis.identifier()) + " cluster coordinator";
            ...
            if (s.ft.clusterCoordinatorIdent.equals(s.comm.ibis.identifier())) {
                ...
                ftLogger.info(
                        "cluster coordinator for cluster " + Victim.clusterOf(s.comm.ibis.identifier()) + " is " + s.ft.clusterCoordinatorIdent);
            }
        } catch (Exception e) {
            ftLogger.error("SATIN '" + s.ident + "': Could not start ibis: " + e, e);
            System.exit(1); // Could not start ibis
        }
    }

    /**
     * If the job is being redone (redone flag is set to true), perform a lookup in
     * the global result table. The lookup might fail if the result is thrown away
     * for some reason, or if the node that stored the result has crashed.
     *
    ...
    protected boolean askForJobResult(InvocationRecord r) {
        ...
        if (value.type == GlobalResultTableValue.TYPE_POINTER) {
            ...
            synchronized (s) {
                ...
                grtLogger.debug("SATIN '" + s.ident + "': sending a result request of " + r.getStamp() + " to " + value.owner);
            ...
            try {
                ...
                grtLogger.warn("SATIN '" + s.ident + "': trying to send RESULT_REQUEST but got " + "exception: " + e, e);
                synchronized (s) {
    ...
    protected void sendAbortAndStoreMessage(InvocationRecord r) {
        ...
        abortLogger.debug("SATIN '" + s.ident + ": sending abort and store message to: " + r.getStealer() + " for job " + r.getStamp());
        ...
        try {
            ...
            ftLogger.warn("SATIN '" + s.ident + "': Got Exception while sending abort message: " + e);
    ...
    protected void sendAbortMessage(InvocationRecord r) {
        ...
        abortLogger.debug("SATIN '" + s.ident + ": sending abort message to: " + r.getStealer() + " for job " + r.getStamp());
    ...
    public void lostConnection(ReceivePort me, SendPortIdentifier johnDoe, Throwable reason) {
        if (reason == null) {
            if (ftLogger.isInfoEnabled()) {
                ftLogger.info("SATIN '" + s.ident + "': got lostConnection upcall: " + johnDoe.ibisIdentifier() + " closed connection");
            }
            return;
        }
        if (ftLogger.isInfoEnabled()) {
            ftLogger.info("SATIN '" + s.ident + "': got lostConnection upcall: " + johnDoe.ibisIdentifier() + ", reason = " + reason, reason);
        }
    ...
    public void lostConnection(SendPort me, ReceivePortIdentifier johnDoe, Throwable reason) {
        if (ftLogger.isInfoEnabled()) {
            ftLogger.info("SATIN '" + s.ident + "': got SENDPORT lostConnection upcall: " + johnDoe.ibisIdentifier(), reason);
        }
        if (connectionUpcallsDisabled) {
            return;
        }
    ...
    protected void handleJoins(IbisIdentifier[] joiners) {
        ftLogger.debug("SATIN '" + s.ident + "': dealing with " + joiners.length + " joins");
        ...
        for (IbisIdentifier joiner : joiners) {
            ...
            try {
                ...
                ftLogger.warn("SATIN '" + s.ident + "': got an exception in Satin.join", e);
                continue;
            }
    ...
    protected void handleAbortAndStore(ReadMessage m) {
        try {
            ...
            grtLogger.error("SATIN '" + s.ident + "': got exception while reading abort_and_store: " + e, e);
        }
    }
        ...
        try {
            ...
            synchronized (s) {
                ...
                if (ASSERTS && value == null) {
                    grtLogger.error("SATIN '" + s.ident + "': EEK!!! no requested result in the table: " + stamp);
                    System.exit(1); // Failed assertion
                } else if (ASSERTS && value.type == GlobalResultTableValue.TYPE_POINTER) {
                    grtLogger.error("SATIN '" + s.ident + "': EEK!!! " + ident + " requested a result: " + stamp
            ...
            if (v == null) {
                ftLogger.debug("SATIN '" + s.ident + "': the node requesting a result died");
                return;
            }
            ...
            grtLogger.error("SATIN '" + s.ident + "': trying to send result back, but got exception: " + e, e);
        } finally {
    ...
    protected void handleResultPush(ReadMessage m) {
        ...
        try {
            ...
            grtLogger.error("SATIN '" + s.ident + "': trying to read result push, but got exception: " + e, e);
        }
    ...
    protected void pushResults(Victim victim, Map<Stamp, GlobalResultTableValue> toPush) {
        ...
        try {
            ...
            if (grtLogger.isDebugEnabled()) {
                grtLogger.debug("SATIN '" + s.ident + "': " + numBytes + " bytes pushed");
            }
        } catch (IOException e) {
            if (m != null) {
                m.finish(e);
            ...
            if (e instanceof NotSerializableException || e instanceof InvalidClassException) {
                grtLogger.warn("SATIN '" + s.ident + "': error pushing results ", e);
            } else if (grtLogger.isInfoEnabled()) {
                grtLogger.info("SATIN '" + s.ident + "': error pushing results " + e);
            }
        }
    }
    ...
    public void handleFileWriteTime(ReadMessage m) {
        ...
        if (s.ft.totalFileWriteInfoMsgs >= s.victims.size() / 2 && s.ft.coordinatorIdent == null && s.ft.tempCoordinatorIdent != null) {
