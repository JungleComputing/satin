...
public final class FaultTolerance implements Config {
    ...
    /**
     * True if the node needs to download the contents of the global result table;
     * protected by the Satin lock.
     */
     ...
     */
    protected ArrayList<IbisIdentifier> crashedIbises = new ArrayList<>();
        ...
        if (CHECKPOINTING) {
            ...
            if (s.isMaster()) {
                ...
                try {
                    ...
                    System.out.println("Sending FILE_WRITE_TIME failed to " + s.getMasterIdent() + ": " + e);
                }
            }
        }
    }
    ...
    public void handleCrashes() {
        ...
        try {
            ...
            synchronized (s) {
                crashesToHandle = new ArrayList<>(crashedIbises);
                crashedIbises.clear();
                gotCrashes = false;
            }

            crashedCopy = new HashSet<>(crashedIbises);
            ...
            for (IbisIdentifier id : crashesToHandle) {
                ...
                if (id.equals(s.getMasterIdent()) || id.equals(clusterCoordinatorIdent)) {
                    try {
                        ...
                        // ignore exception
                        ftLogger.info("SATIN '" + s.ident + "' :exception while notifying registry about " + "crash of " + id + ": " + e, e);
                    }
                }
            }
            ...
            synchronized (s) {
                while (crashesToHandle.size() > 0) {
                    ...
                    ftLogger.debug("SATIN '" + s.ident + ": handling crash of " + id);
    ...
    public void handleMasterCrash() {
        ftLogger.info("SATIN '" + s.ident + "': MASTER (" + s.getMasterIdent() + ") HAS CRASHED");
        ...
        try {
            ...
            ftLogger.error("SATIN '" + s.ident + "' :exception while electing a new master " + e, e);
            System.exit(1);
        }
        ...
        synchronized (s) {
            ...
            if (s.getMasterIdent().equals(s.ident)) {
                ...
                ftLogger.info("SATIN '" + s.ident + "': " + s.getMasterIdent() + "is the new master");
            }
            restarted = true;
        }
    }
    ...
    public void handleClusterCoordinatorCrash() {
        ...
        try {
            ...
            ftLogger.warn("SATIN '" + s.ident + "' :exception while electing a new cluster coordinator " + e, e);
        }
    }
    ...
    public void killAndStoreChildrenOf(Stamp targetStamp) {
        ...
        for (InvocationRecord element : toStore) {
            storeFinishedChildrenOf(element);
        }
    ...
    private void pushJobs(Victim v) {
        Map<Stamp, GlobalResultTableValue> toPush = new HashMap<>();
        synchronized (s) {
            ArrayList<InvocationRecord> tmp = s.onStack.getAllFinishedChildren(v);

            for (InvocationRecord curr : tmp) {
                Stamp key = curr.getStamp();
                GlobalResultTableValue value = new GlobalResultTableValue(GlobalResultTableValue.TYPE_RESULT, curr);
    ...
    public void handleDelayedMessages() {
        ...
        if (CHECKPOINTING) {
            ...
            if (takeCheckpoint) {
                /*
                 * if (coordinator && !CHECKPOINT_PUSH){ broadcastCheckpointRequest();
                 * takeCoordinatorCheckpoint(); } else if (coordinator && CHECKPOINT_PUSH){
                 * takeCoordinatorCheckpoint(); } else if (!coordinator && CHECKPOINT_PUSH){
    ...
    void handleCheckpointAndQuit() {
        synchronized (s) {
            if (coordinator) {
                ...
                System.out.println("SATIN '" + s.ident + "': coordinator quits");
            } else {
                takeAndSendCheckpoint();
                System.out.println("SATIN '" + s.ident + "': checkpoint taken");
            }
    ...
    public void broadcastCheckpointRequest() {
        ...
        for (int i = 0; i < size; i++) {
            ...
            try {
                ...
                System.out.println("sending CHECKPOINT_REQUEST failed to " + victim.getIdent() + ": " + e);
            }
        }
    ...
    /**
     * Retrieve checkpoints from local queue, and store them in the global variable
     * 'checkpoints'
     **/
    ...
    public void takeAndSendCheckpoint() {
        if (coordinator || becomeCoordinator || findNewCoordinator || setCoordinator) {
        ...
        try {
            ...
            System.out.println("sending CHECKPOINT failed to " + coordinatorIdent + ": " + e);
        }
        s.stats.makeCheckpointTimer.stop();
    }
    ...
    public void broadcastCheckpointInfo() {
        ...
        for (int i = 0; i < size; i++) {
            ...
            try {
                ...
                System.out.println("sending CHECKPOINT_INFO failed to " + v.getIdent() + ": " + e);
            }
        }
    }
    ...
    public int computeConnectionSpeed() {
        ...
        try {
            FileOutputStream outFile = GAT.createFileOutputStream(context, new URI(filename));
    ...
    /**
     * Makes the tempCoordinator (i.e. the node which has the fastes filewrite time
     * uptill now) the coordinator and lets the other nodes know about the new
     * coordinator
     **/
    public void setCoordinator() {
        ...
        for (int i = 0; i < size; i++) {
            ...
            try {
                ...
                System.out.println("sending COORDINATOR_INFO failed to " + v.getIdent() + ": " + e);
            }
        }
    ...
    /**
     * Makes this node coordinator: - start the necessairy threads - initialize the
     * checkpoint file - broadcast checkpoint info to other nodes
     **/
    public void coordinatorInit() {
        ...
        checkpointFile = new CheckpointFile(CHECKPOINT_FILE, CHECKPOINT_MAXFILESIZE);
    ...
    public void findNewCoordinator() {
        ...
        for (int i = 0; i < size; i++) {
            ...
            try {
                ...
                System.out.println("sending FILE_WRITE_TIME_REQ failed to " + v.getIdent() + ": " + e);
            }
        }
        s.stats.createCoordinatorTimer.stop();
    }

    public boolean checkForDuplicateWork(InvocationRecord parent, InvocationRecord r) {
        if (FT_NAIVE) {
            return false;
        }
