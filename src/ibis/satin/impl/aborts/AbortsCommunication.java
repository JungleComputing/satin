...
final class AbortsCommunication implements Config {
    ...
    private ArrayList<StampListElement> stampsToAbortList = new ArrayList<>();
    ...
    protected void sendAbortMessage(InvocationRecord r) {
        ...
        abortLogger.debug("SATIN '" + s.ident + ": sending abort message to: " + r.getStealer() + " for job " + r.getStamp() + ", parent = "
                + r.getParentStamp());
    ...
    protected void soRealSendAbortMessage(StampListElement e) {
        ...
        try {
            ...
            abortLogger.warn("SATIN '" + s.ident + "': Got Exception while sending abort message (continuing): " + x, x);
    ...
    protected void handleAbort(ReadMessage m) {
        try {
            ...
            abortLogger.error("SATIN '" + s.ident + "': got exception while reading job result: " + e1, e1);
        }
    }
}
