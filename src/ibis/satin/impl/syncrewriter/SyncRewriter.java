...
public class SyncRewriter extends IbiscComponent {
    ...
    static final String[] ANALYZER_NAMES = { "Naive", "EarliestLoad", "ControlFlow" };
    ...
    void dump(JavaClass spawnableClass) {
        String fileName = spawnableClass.getClassName() + (RETAIN_ORIGINAL_CLASSFILES ? "_.class" : ".class");
    ...
    void rewrite(String className, SpawnSignature[] spawnSignatures) throws NoSpawningClassException, ClassRewriteFailure, AssumptionFailure {
    ...
    void rewrite(JavaClass javaClass, SpawnSignature[] spawnSignatures) throws NoSpawningClassException, ClassRewriteFailure, AssumptionFailure {
    ...
    void setAnalyzer(String analyzerName) {
        try {
            analyzer = AnalyzerFactory.createAnalyzer(analyzerName);
        } catch (IllegalAccessException | IllegalArgumentException | ClassNotFoundException | NoSuchMethodException | InstantiationException
    ...
    void start(String[] argv) {
        ...
        for (String classFileName : classNames) {
            ...
            try {
                ...
                d.error("%s has spawns that don't return a value or throw an inlet." + " The syncrewriter cannot handle this.", className);
                System.exit(1);
            }
        }
        ...
        while (classes.hasNext()) {
            ...
            if (classNames.contains(clazz.getClassName())) {
                try {
                    ...
                    d.error("%s has spawns that don't return a value or throw an inlet." + " The syncRewriter cannot handle this.",
                            clazz.getClassName());
                    System.exit(1);
                }
            }
