...
class SpawningClass extends ClassGen {
    ...
    private void adviseForSpawnSignature(SpawnSignature spawnSignature, Analyzer analyzer) {
        ...
        for (Method method : methods) {
            try {
                SpawningMethod spawningMethod = new SpawningMethod(method, getClassName(), getConstantPool(), spawnSignature, 0,
     ...
     */
    private void rewriteForSpawnSignature(SpawnSignature spawnSignature, Analyzer analyzer) throws MethodRewriteFailure {
        boolean throwRewriteFailure = false;

        d.log(0, "rewriting for %s\n", spawnSignature);

        int indexSync = getConstantPool().addMethodref(spawnSignature.getClassName(), "sync", "()V");

        Method[] methods = getMethods();

        for (Method method : methods) {
            try {
                SpawningMethod spawningMethod = new SpawningMethod(method, getClassName(), getConstantPool(), spawnSignature, indexSync,
                        new Debug(d.turnedOn(), d.getStartLevel() + 2));
                d.log(1, "%s calls %s\n", method.getName(), spawnSignature);
                ...
                System.out.println("Method " + method.getName() + " in class " + getClassName() + " already contains a sync call");
            } catch (NoSpawningMethodException e) {
                d.log(1, "%s doesn't call %s\n", method.getName(), spawnSignature);
                // spawnSignature is not called
            } catch (MethodRewriteFailure e) {
    ...
    /*
     * it is true that creating a spawning method happens twice, once to find the
     * spawnsignatures for this class and once for rewriting the methods. It seems a
     * better idea to just store spawningMethods in the beginning and rewrite that.
     * However, every time you rewrite a spawningMethod for 'spawnSignature1' and
     * then you would have to update all these spawningMethods for the method that
     * has been rewritten, in case you are going to rewrite for 'spawnSignature2'.
     * Rewriting is an iterative process:
     *
     ...
     * this.add(spawningMethod.getMethod());
     *
     * Therefore, the easiest option is chosen: Just try to create a SpawningMethod
     * for some spawnSignature and record the spawnSignature.
     */
    private void getSpawnSignatures(ArrayList<SpawnSignature> spawnSignatures, SpawnSignature spawnSignature) throws AssumptionFailure {
        ...
        for (Method method : methods) {
            try {
                ...
                new SpawningMethod(method, getClassName(), getConstantPool(), spawnSignature, -1 /* doesn't matter for this time */, new Debug());
    ...
    private SpawnSignature[] getSpawnSignatures(SpawnSignature[] allSpawnSignatures) throws AssumptionFailure {
        ArrayList<SpawnSignature> spawnSignatures = new ArrayList<>();
