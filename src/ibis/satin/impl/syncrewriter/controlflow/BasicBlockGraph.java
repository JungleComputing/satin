...
public class BasicBlockGraph {
     ...
     *
     * @param id the identifier of the basic block from which all ending paths have
     *           to be calculated.
    ...
    private void calculateEndingPaths(ArrayList<Path> paths, BasicBlock current, Path visited) {
        if (current.isEnding()) {
            visited.add(current);
    ...
    private void addBasicBlock(Path basicBlocks, InstructionContext start, ArrayList<InstructionContext> instructions, InstructionContext end,
            MethodGen methodGen) {
        if (start == null || instructions == null || end == null) {
            throw new Error("BasicBlock start/end out of sync");
        } else {
            basicBlocks.add(new BasicBlock(start, instructions, end, basicBlocks.size(), methodGen));
        }
    }

    private boolean isEndInstruction(InstructionContext context) {
        return context.getInstruction().getInstruction() instanceof ReturnInstruction || context.getInstruction().getInstruction() instanceof ATHROW;
    }

    private boolean isTarget(InstructionContext context, ArrayList<InstructionContext> targets) {
    ...
    private boolean isEndOfBasicBlock(InstructionContext currentContext, ArrayList<InstructionContext> targets, InstructionContext nextContext,
        ...
        return hasOneSuccessorNotBeingNext(nextContext, successors) || successors.length > 1 || isTarget(nextContext, targets)
    ...
    private boolean isStartOfBasicBlock(int i, InstructionContext currentContext, ArrayList<InstructionContext> targets) {
        return i == 0 || targets.contains(currentContext);
    }

    /* add the exception handlers to the targets */
    private void addExceptionHandlers(InstructionContext instructionContext, ArrayList<InstructionContext> targets, ControlFlowGraph graph) {
        ExceptionHandler[] handlers = instructionContext.getExceptionHandlers();
        for (ExceptionHandler handler : handlers) {
            InstructionContext handlerContext = graph.contextOf(handler.getHandlerStart());
    ...
    private ArrayList<InstructionContext> getTargets(InstructionContext[] contexts, ControlFlowGraph graph) {

        ArrayList<InstructionContext> targets = new ArrayList<>();

        for (int i = 0; i < contexts.length; i++) {
            InstructionContext currentContext = contexts[i];
            InstructionContext nextContext = i < contexts.length - 1 ? contexts[i + 1] : null;
    ...
    private InstructionContext[] getContexts(MethodGen methodGen, ControlFlowGraph graph) {
    ...
    /*
     * constructs basic basic blocks, without the targets and the levels set right.
     */
    private Path constructBasicBasicBlocks(MethodGen methodGen) {
        ...
        for (int i = 0; i < contexts.length; i++) {
            ...
            if (isEndOfBasicBlock(currentContext, targets, nextContext, successors)) {
                checkIfSet(start, instructions, !SHOULD_BE_NULL);
                addBasicBlock(basicBlocks, start, instructions, currentContext, methodGen);
                start = null;
                instructions = null;
            }
        }
