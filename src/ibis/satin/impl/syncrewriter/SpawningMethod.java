...
public class SpawningMethod extends MethodGen {
    ...
    private static class ResultNotStored extends Exception {
    ...
    SpawningMethod(Method method, String className, ConstantPoolGen constantPoolGen, SpawnSignature spawnSignature, int indexSync, Debug d)
    ...
    void adviseSync(Analyzer analyzer) throws SyncInsertionProposalFailure {
        ...
        InstructionHandle[] ihs = analyzer.proposeSyncInsertion(this, new Debug(d.turnedOn(), d.getStartLevel() + 2));
        ...
        for (InstructionHandle ih : ihs) {
            ...
            System.out.println("Insert a sync() in method " + getName() + " at line " + l + " in class " + getClassName());
        }
    }
    ...
    private void insertSync(Analyzer analyzer) throws MethodRewriteFailure {
        ...
        for (InstructionHandle ih : ihs) {
            InstructionHandle syncInvoke = instructionList.insert(ih, new INVOKEVIRTUAL(indexSync));
            InstructionHandle newTarget = instructionList.insert(syncInvoke, spawnableCalls.get(0).getObjectReference().getInstruction());
    ...
    private boolean callsSync() {
        ...
        while (handle != null) {
            ...
            if (ins instanceof INVOKEVIRTUAL) {
                ...
                if (inv.getMethodName(cpg).equals("sync") && inv.getSignature(cpg).equals("()V")) {
    ...
    private ArrayList<CodeExceptionGen> getExceptionHandlers(InstructionHandle ih, MethodGen spawnSignature) {
        ...
        ArrayList<CodeExceptionGen> result = new ArrayList<>();

        for (CodeExceptionGen codeException : codeExceptions) {
            // codeException.containsTarget has a BUG????
            /*
             * if (codeException.containsTarget(ih) && hasRightType(codeException,
             * spawnSignature)) { result.add(codeException); }
             */
    ...
    private void getIndicesStores(InstructionHandle start, InstructionHandle end, ArrayList<LocalVariableGen> resultIndices) {
    ...
    private SpawnableCall getSpawnableCallWithException(InstructionHandle invokeInstruction, ArrayList<CodeExceptionGen> exceptionHandlers) {

        ArrayList<LocalVariableGen> resultIndices = new ArrayList<>();
        ...
        return new SpawnableCall(invokeInstruction, getObjectReferenceLoadInstruction(invokeInstruction), resultIndices.toArray(dummy));
    }
    ...
    private SpawnableCall getSpawnableCallReturningValue(InstructionHandle invokeInstruction) {
        try {
            return new SpawnableCall(invokeInstruction, getObjectReferenceLoadInstruction(invokeInstruction), findIndexStore(invokeInstruction));
        } catch (ResultNotStored e) {
            return new SpawnableCall(invokeInstruction, getObjectReferenceLoadInstruction(invokeInstruction), SpawnableCall.Type.RESULT_NOT_STORED);
        }
    }

    private ArrayList<SpawnableCall> getSpawnableCalls(ConstantPoolGen constantPoolGen, MethodGen spawnSignatureGen) throws AssumptionFailure {

        ArrayList<SpawnableCall> spawnableCalls = new ArrayList<>();

        InstructionList il = getInstructionList();
        int indexSpawnableCall = constantPoolGen.lookupMethodref(spawnSignatureGen);
