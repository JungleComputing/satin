...
public class ControlFlow implements Analyzer {
    ...
    private static class PathPartOfLoopException extends Exception {
    ...
    private static class ResultNotLoadedException extends Exception {
        private static final long serialVersionUID = 1L;
     ...
     *
     * If the result of a spawnable method is not stored, but given to a method for
     * example, this analyzer can't do its job and it will propose syncs right after
     * the spawnable calls.
     *
     * It tries to find all store-to-load paths for every spawnable call and will
     * then try to find the earliest load in the last basic blocks of those
     * store-to-load paths.
     *
     ...
     * spawnable calls.
     *
     * @param spawningMethod The spawnable method in which syncs should be inserted.
     * @param d              A debug instance for logging error, warning and debug
     *                       messages.
     */
    @Override
    public InstructionHandle[] proposeSyncInsertion(SpawningMethod spawningMethod, Debug d) throws SyncInsertionProposalFailure {
    ...
    private void removeSpawnableCallsBefore(InstructionHandle ih, ArrayList<SpawnableCall> spawnableCalls, BasicBlock basicBlock) {
        ...
        ArrayList<SpawnableCall> spawnableCallsClone = new ArrayList<>(spawnableCalls);
     ...
     * statement for these spawnable calls as well. So add those earliest loads as
     * well.
     *
     ...
     *
     * read(result3); // this is also an earliest load, // this is the one that is
     * returned
     */
    private void addLoadsWithSpawnableCallsAfter(InstructionHandle earliestLoad, BasicBlock basicBlock, SpawningMethod spawningMethod,
            ArrayList<SpawnableCall> spawnableCalls, ArrayList<InstructionHandle> earliestLoads) {

        ArrayList<SpawnableCall> spawnableCallsCopy = new ArrayList<>(spawnableCalls);
        ...
        while (spawnableCallsCopy.size() > 0) {
            ...
            try {
                earliestLoad = getEarliestLoadInstruction(basicBlock, spawnableCallsCopy, spawningMethod);
     ...
     * spawnable call.
     *
     * if the spawnable call isn't invoked, 0, the first instruction is returned.
     */
    ...
    /*
     * Get the load instruction from a basic block associated with a spawnable call.
     *
     ...
     */
    private InstructionHandle getLoadInstruction(BasicBlock basicBlock, SpawnableCall spawnableCall, SpawningMethod spawningMethod)
        ...
        for (int i = indexAfterSpawnableCall; i < instructions.size(); i++) {
            InstructionHandle ih = instructions.get(i).getInstruction();
            try {
                INVOKESTATIC pauseInstruction = (INVOKESTATIC) (ih.getInstruction());
                if (pauseInstruction.getMethodName(cp).equals("pause") && pauseInstruction.getType(cp).getSignature().equals("V")
            ...
            try {
                ...
                if (spawnableCall.storesIn(loadInstruction.getIndex(), ih) && !spawningMethod.isUsedForArrayLength(ih)
                        && !spawningMethod.isUsedForArrayStore(ih) && !spawningMethod.isUsedForPutField(ih)) {
                    return ih;
                }
            } catch (ClassCastException e) {
            }
     ...
     */
    private InstructionHandle getEarliestLoadInstruction(BasicBlock basicBlock, ArrayList<SpawnableCall> spawnableCalls,
        ...
        for (SpawnableCall spawnableCall : spawnableCalls) {
            ...
            try {
                ...
                if (earliestLoadInstruction == null || loadInstruction.getPosition() < earliestLoadInstruction.getPosition()) {
     ...
     *
     * If no earliest load is found, default to the last instruction of the basic
     * block.
     */
    private ArrayList<InstructionHandle> tryEarliestLoadInstructions(BasicBlock basicBlock, SpawningMethod spawningMethod) {
        ...
        ArrayList<InstructionHandle> earliestLoads = new ArrayList<>();

        try {
            d.log(1, "trying to get the earliest load associated with spawnable calls in basic block %d\n", basicBlock.getId());
            InstructionHandle earliestLoad = getEarliestLoadInstruction(basicBlock, spawnableCalls, spawningMethod);
            earliestLoads.add(earliestLoad);
    ...
    private InstructionHandle[] getEarliestLoadInstructions(BasicBlock[] basicBlocks, SpawningMethod spawningMethod) {
        ArrayList<InstructionHandle> bestInstructionHandles = new ArrayList<>();

        d.log(0, "trying to get the earliest load instructions\n");
        for (BasicBlock basicBlock : basicBlocks) {
            bestInstructionHandles.addAll(tryEarliestLoadInstructions(basicBlock, spawningMethod));
        }
    ...
    private ArrayList<BasicBlock> removeDuplicates(ArrayList<BasicBlock> basicBlocks) {
        ArrayList<BasicBlock> withoutDuplicates = new ArrayList<>();
        for (BasicBlock basicBlock : basicBlocks) {
    ...
    /*
     * filters away the basic blocks that can be reached by other basic blocks in
     * any case
     */
    private ArrayList<BasicBlock> filter(ArrayList<BasicBlock> basicBlocks, ArrayList<Path> storeLoadPaths) {
        ArrayList<BasicBlock> filteredBasicBlocks = new ArrayList<>();
        ...
        for (BasicBlock element : basicBlocksArray) {
            if (element != null) {
                filteredBasicBlocks.add(element);
            }
        }
     ...
     *
     * The best basic block can be the last basic block that is not part of a loop,
     * otherwise the last basic block of the path.
     */
     ...
     *
     * This can be the last basic block that is not part of a loop, otherwise the
     * last basic block of the path.
     */
    private BasicBlock getBestBasicBlock(BasicBlockGraph basicBlockGraph, Path path) {
        try {
            BasicBlock lastBasicBlockNotPartOfLoop = getLastBasicBlockNotPartOfLoop(basicBlockGraph, path);
    ...
    private ArrayList<Path> getStoreLoadPaths(SpawnableCallAnalysis[] spawnableCallAnalyses) {
        ArrayList<Path> allStoreLoadPaths = new ArrayList<>();
    ...
    private SpawnableCallAnalysis[] getSpawnableCallAnalyses(ArrayList<SpawnableCall> spawnableCalls, BasicBlockGraph basicBlockGraph) {
        ...
        for (int i = 0; i < spawnableCalls.size(); i++) {
            spawnableCallAnalyses[i] = new SpawnableCallAnalysis(spawnableCalls.get(i), basicBlockGraph,
                    new Debug(d.turnedOn(), d.getStartLevel() + 1));
        }
     ...
     *
     * Try to get the best basic block, block not within a loop Filter out the basic
     * blocks that always can be reached by a previous block Again remove duplicates
     */
    private BasicBlock[] proposeBasicBlocks(SpawningMethod spawningMethod) throws SyncInsertionProposalFailure {
     ...
     * basicBlocks = new BasicBlock[spawnableCallAnalyses.length]; for (int i = 0; i
     * < basicBlocks.length; i++) { basicBlocks[i] =
     ...
     * StringBuilder sb = new StringBuilder("["); for (int i = 0; i < paths.length -
     * 1; i++) { sb.append(paths[i]); sb.append(", "); }
     ...
     * BasicBlock[] basicBlocks = new BasicBlock[paths.size()]; for (int i = 0; i <
     * paths.size(); i++) { basicBlocks[i] = paths.get(i).getLastBasicBlock(); }
     *
     * return basicBlocks; }
     */
}
