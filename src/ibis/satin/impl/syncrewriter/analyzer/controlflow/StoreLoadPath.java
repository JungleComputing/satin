...
public class StoreLoadPath extends Path {
    ...
    StoreLoadPath(InstructionHandle storeInstruction, Path path, LocalVariableGen[] localVariableIndices) throws NeverReadException {
        super(path);
    ...
    private int getIndexPauseBasicBlock(InstructionHandle storeInstruction) throws NeverReadException {
        for (int i = 0; i < size(); i++) {
            ...
            } else if (basicBlock.containsPause()) {
                return i;
            }
        }
        throw new NeverReadException();
    ...
    private boolean containsLoadWithIndexAfter(LoadAwareBasicBlock basicBlock, LocalVariableGen[] localVariableIndices,
    ...
    private int getIndexEarliestBasicBlock(LocalVariableGen[] localVariableIndices, InstructionHandle storeInstruction) throws NeverReadException {
        for (int i = 0; i < size(); i++) {
            ...
            } else if (containsLoadWithIndex(basicBlock, localVariableIndices)) {
                return i;
            }
        }
        throw new NeverReadException();
