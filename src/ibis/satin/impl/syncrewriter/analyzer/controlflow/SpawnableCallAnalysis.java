...
public class SpawnableCallAnalysis {
    ...
    /**
     * Instantiates a spawnable call analysys from a spawnable call and a graph of
     * basic blocks.
     *
    ...
    public SpawnableCallAnalysis(SpawnableCall spawnableCall, BasicBlockGraph basicBlockGraph, Debug d) {
        ...
        if (lt == null) {
            d.warning("Could not get local variable table, did you " + "compile with the '-g' option?");
        }
        ...
        if (storeLoadPaths.size() == 0) {
            ...
            latestCommonSubPath = Path.getLatestCommonSubPath(endingPaths.toArray(endingPathsArray));
        } else {
            Path[] storeLoadPathsArray = new Path[storeLoadPaths.size()];
            latestCommonSubPath = Path.getLatestCommonSubPath(storeLoadPaths.toArray(storeLoadPathsArray));
        }
        /*
     ...
     *
     * If there are no store-to-load paths, the latest common subpaths of all ending
     * paths is taken.
     *
    ...
    private ArrayList<StoreLoadPath> getStoreLoadPaths(ArrayList<Path> endingPaths, SpawnableCall spawnableCall) {

        ArrayList<StoreLoadPath> storeLoadPaths = new ArrayList<>();
