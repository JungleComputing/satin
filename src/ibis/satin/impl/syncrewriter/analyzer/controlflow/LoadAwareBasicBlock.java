...
class LoadAwareBasicBlock extends BasicBlock {
    ...
    /*
     * tests whether this basic block contains load with index localVarIndex after
    ...
    boolean noAliasesLoadWithIndexBefore(InstructionHandle ih, LocalVariableGen lg) {
        for (InstructionContext ic : instructions) {
            ...
            if (l == lg && methodGen.instructionLoadsTo(current, lg.getIndex())) {
                ...
                if (!methodGen.isUsedForArrayLoad(current) && !methodGen.isUsedForArrayLength(current) && !methodGen.isUsedForArrayStore(current)
                        && !methodGen.isUsedForPutField(current) && !methodGen.isUsedForGetField(current)) {
                    return false;
                }
            }
        }
            ...
            if (methodGen.instructionStoresTo(current, lg.getIndex())) {
                ...
                if (prev != null) {
                    ...
                    if (i instanceof INVOKESPECIAL) {
                        ...
                        if (invoker.getMethodName(methodGen.getConstantPool()).equals("<init>") && isNonEscapingConstructor(invoker)) {
                            continue;
                        }
                    }
                    ...
                    if (i instanceof NEWARRAY || i instanceof ANEWARRAY || i instanceof MULTIANEWARRAY || i instanceof ConstantPushInstruction
    ...
    boolean nonEscapingConstructor(JavaClass javaClass, String constructorSignature) {
        ...
        try {
            ...
            throw new Error("Superclass of " + javaClass.getClassName() + " not found");
        }
