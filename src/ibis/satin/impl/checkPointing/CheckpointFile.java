...
public class CheckpointFile {
    ...
    private ArrayList<Stamp> checkpoints = new ArrayList<>();
    ...
    /**
     * Writes newCheckpoints to the checkpointFile and checks the filesize of the
     * checkpointfile for compression afterwards. Every exception is fatal and
     * results in no write-possibilities in the future whatsoever
     **/
    public void write(ArrayList<Checkpoint> newCheckpoints) {
        if (stopWriting || fileStream == null || fileWriter == null) {
            return;
        }
        ...
        try {
            GATContext context = new GATContext();
            if (maxFileSize > 0 && GAT.createFile(context, new URI(filename)).length() > maxFileSize) {
                compress();
    ...
    /**
     * Retrieves all the checkpoints which belonged to 'id' from the checkpoint-file
     * and stores them in 'grt'. if id == null, all checkpoints belonging to any
     * node will be stored in 'grt.'
     */
    public int read(Set<IbisIdentifier> id, GlobalResultTable grt) {
        ...
        try {
            GATContext context = new GATContext();
            tempInputFile = GAT.createFileInputStream(context, new URI(filename));
    ...
    /**
     * Tries to compress the checkpointfile. Every exception is fatal and results in
     * no write-possibilities in the future whatsoever
     **/
    ...
    /**
     * Tries to restore the old checkpointFile. Every Exception is fatal and leads
     * to no write-possibiliets in the future whatsoever
     **/
    ...
    /**
     * First finds all the checkpoints which don't have any parents which are also
     * available, and copies only these checkpoints to dest.
     **/
    private void compress(String source, String dest) throws Exception {
        ...
        while (i < checkpoints.size()) {
            ...
            while (j < checkpoints.size()) {
                ...
                if (stamp2 == null || stamp2.equals(stamp1) || stamp2.isDescendentOf(stamp1)) {
        ...
        GATContext context = new GATContext();
        tempOutputFile = GAT.createFileOutputStream(context, new URI(dest), false);
    ...
    /**
     * Copies all the objects from the old checkpointfile to the new copy until a
     * StreamCorruptedException occurs. Everything that was written after this
     * points is lost.
     **/
    private void restore(String source, String dest, GlobalResultTable grt) throws Exception {
        FileOutputStream tempOutputFile = null;
        ObjectOutputStream tempWriter = null;
        FileInputStream tempInputFile = null;
        ObjectInputStream tempReader = null;
        try {
                ...
                if (grt != null) {
                    ...
                    try {
                        if (checkpoints.size() % 1000 == 999) {
                            ...
                            System.out.println("update took " + (end - begin) + " ms");
                        }
                    } catch (Exception e) {
            ...
            System.out.println("restored " + checkpoints.size() + " from a corrupted checkpoint file");
        }
        try {
            fileStream.close();
            fileWriter.close();
    ...
    /**
     * Tries to open filename, so that it will be used for future write-operations
     **/
    private void open(String filename) throws Exception {
        GATContext context = new GATContext();
        fileStream = GAT.createFileOutputStream(context, new URI(filename), false);
        fileWriter = new ObjectOutputStream(fileStream);
    }

    /**
     * Prints Exception e, closes all streams and deletes all temporary files.
     * Cosequently, all future write-operations will fail. The read-operations might
     * still succeed if fatal wasn't called on a moment the original file (i.e.
